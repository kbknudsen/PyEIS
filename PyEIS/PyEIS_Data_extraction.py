...
def extract_mpt(path, EIS_name):
    ...
    Kristian B. Knudsen (kknu@berkeley.edu || kristianbknudsen@gmail.com)
    """
    EIS_init = pd.read_csv(
        path + EIS_name, sep="\t", nrows=1, header=0, names=["err"], encoding="latin1"
    ...
    EIS_test_header_names = pd.read_csv(
        path + EIS_name,
        sep="\t",
        skiprows=int(EIS_init.err[0][18:-1]) - 1,
        encoding="latin1",
    )  # locates number of skiplines
    names_EIS = []
    for j in range(len(EIS_test_header_names.columns)):
        names_EIS.append(
            correct_text_EIS(EIS_test_header_names.columns[j])
        )  # reads coloumn text
